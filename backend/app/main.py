...
def expand_node(node_id: str, req: NodeExpandRequest):
    ...
    # Retrieve compact candidate context (~80-140 tokens)
    ...
    if req.expansion_type == "topics_affecting_question" or node["node_type"] == "question":
        # Deep Question Decomposition with Pre-Existing Node Awareness
        ...
        with conn:
            # 1. Connect to identified pre-existing nodes
            ...
            # 2. Process new nodes with deduplication guard
            new_title_to_id = {}
            for i, topic_item in enumerate(getattr(decomp, "new_nodes", [])):
                ...
                if is_dup and dup_id and dup_id != node_id:
                    # Link to existing node instead of creating a redundant node!
            ...
            # 3. Cross-links between new nodes and pre-existing nodes
        ...
        # Standard Concept Subtopics expansion with Pre-Existing Node Awareness
        ...
        with conn:
            # 1. Connect to pre-existing nodes identified by AI
            ...
            # 2. Process newly suggested subtopics with deduplication guard
            new_title_to_id = {}
            for i, item in enumerate(getattr(expansion, "new_nodes", [])):
                ...
                if is_dup and dup_id and dup_id != node_id:
                    # Deduplicate: Connect to existing node!
            ...
            # 3. Cross-links between new subtopics and other pre-existing nodes
...
@app.get("/api/topics/{topic_id}/communities", response_model=CommunityGroupResponse)
...
@app.post("/api/nodes/{node_id}/done", response_model=NodeResponse)
...
@app.post("/api/topics/{topic_id}/auto-organize")
